...
```ts
   ...
   *  - POST {base}/route (form) params: efficiency, range, from, to -> { "job": "<uuid>" }
   *  - GET  {base}/results/{job} -> { "state": "queued"|"completed", "result": {
   *      "system_jumps": [{ "system": s, "distance_jumped": n, "distance_left": n,
   *                          "jumps": n, "neutron_star": bool }, ...],
   *      "total_jumps": n } }
   *    The SOURCE system is included as the first entry with jumps: 0.
   */
  ...
  private mapNeutronResult(raw: any): NeutronRoute {
    ...
    return {
      waypoints,
      totalJumps: raw.total_jumps ?? waypoints.reduce((s, w) => s + w.jumps, 0),
