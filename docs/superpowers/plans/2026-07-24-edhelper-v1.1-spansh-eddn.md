...
/*
 * DECLARED SHAPE (canonical source: packages/app/fixtures/spansh/*.json from Task 2;
 * if the fixtures differ, this mapping was amended by the Task 2 agent):
 *  - POST {base}/trade/route (form-urlencoded) -> { "job": "<uuid>" }
 *  - GET  {base}/results/{job} -> { "state": "queued" | "completed", "result": [hop...] }
 *    hop: { "source": {"system": s, "station": s}, "destination": {"system": s, "station": s},
 *           "distance": n, "commodities": [{"name": s, "amount": n, "buy_price": n,
 *           "sell_price": n, "total_profit": n}], "total_profit": n, "cumulative_profit": n }
...
export class SpanshClient {
  ...
  async plotTrade(req: PlotTradeRequest): Promise<PlotTradeResult> {
    const form = new URLSearchParams({
      ...
      capital: String(req.capital),
      max_hops: String(req.maxHops),
      max_hop_distance: String(req.maxHopDistance),
      cargo: String(req.cargoCapacity),
      max_system_distance: '1000',
      max_price_age: String(req.maxDataAgeDays ?? 30),
  ...
  private mapResult(rawHops: any[], req: PlotTradeRequest): PlotTradeResult {
    const hops: Hop[] = rawHops.map((h) => {
      ...
      return {
        ...
        units: top?.amount ?? 0,
        buyPrice: top?.buy_price ?? 0,
        sellPrice: top?.sell_price ?? 0,
        profit: hopProfit,
