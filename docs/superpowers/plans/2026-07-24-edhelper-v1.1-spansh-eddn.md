...
- **searchSystems/searchStations unwired** — implemented and tested on the host, but no renderer autocomplete/validation consumes them yet (plotTrade resolves names itself). Wire up when the planner form gets autocomplete.
- **Footer toggle accessibility** — the broadcast kill-switch is a styled span with onclick, invisible to the Windows accessibility tree; make it a real button.

- **Outfitting/shipyard EDDN schemas** — deferred by spec.
- **Spansh periodic health probe** — health currently updates only on real calls (the host pushes a 'spansh' event whenever health changes after a request — added in Task 7); an idle app still shows stale "last checked". Add a 10-minute lightweight probe if it bothers.
- **Spansh-500 host integration test** — the client's failure path is unit-tested, but no host-level test drives a Spansh 500 through the RPC surface to a degraded getDataHealth; add a failing mock route when touching host tests next.
- **Tracked-position atomicity** — engine-host updates StarSystem unconditionally but StarPos/SystemAddress only when present; real ED journals always ship them together, but the trio isn't updated atomically.
