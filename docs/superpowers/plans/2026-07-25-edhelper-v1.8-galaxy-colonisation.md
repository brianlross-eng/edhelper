...
- Editing text/numeric inputs after plotting does not clear the stale plotted result in ANY router view (only mode/option chips do) — codebase-wide pattern; harmless (START uses the returned route object) but a UX-truthfulness gap worth a shared fix.
