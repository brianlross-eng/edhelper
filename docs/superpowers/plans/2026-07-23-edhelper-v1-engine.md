...
- **Stale EDDN messages**: applyEddnCommodity doesn't skip messages older than stations.market_updated_at, so replayed EDDN traffic can briefly overwrite fresher prices. (Timestamp formats are now normalized to SQLite canonical UTC via `src/time.ts`, so a future staleness guard can safely string-compare.)
- **Supply/demand not decremented across hops**: planRoute re-reads live listings per hop, so a route that revisits the same station pair (large maxHops) can "re-harvest" finite supply/demand, inflating projected profit. Fine at default maxHops 3-5; revisit if longer routes are exposed.
- **Beam pruning untested**: the fixture's branching factor never exceeds beamWidth, so bestPerStation dedup and slice pruning aren't exercised where they change the outcome. Add a wider fixture when planner work resumes.
